...
PIDS=""; for p in /proc/[0-9]*; do bn=${p##*/}; f="/proc/$bn/comm"; [ -r "$f" ] || continue; c=$(cat "$f"); echo "$c" | grep -Eiq "$PROC_MATCH" || continue; PIDS="$PIDS $bn"; done; echo ${PIDS# }' 2>/dev/null || true)
    [ -z "$PIDS" ] && PIDS="1"
    local INNER_STATIC='read UP < /proc/uptime; SEC=${UP%.*}; FRA=${UP#*.}; FRA=${FRA%% *}; MS=$(printf "%-3s" "$FRA" | cut -c1-3 | tr " " "0"); BT=$(grep -m1 "^btime " /proc/stat | cut -d" " -f2); TS=$(( BT*1000 + SEC*1000 + 10#$MS )); for pid in '$PIDS'; do if [ -r /proc/$pid/stat ]; then LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; RSS=$( (grep -m1 "Rss:" /proc/$pid/smaps_rollup 2>/dev/null | tr -s " " | cut -d" " -f2) || true ); if [ -z "$RSS" ]; then PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null) && RSS=$(( ${PAGES:-0} * 4 )); fi; printf "{\"ts_ms\":%s,\"pid\":%s,\"rss_kb\":%s,\"utime\":%s,\"stime\":%s}\n" "$TS" "$pid" "${RSS:-0}" "${UT:-0}" "${ST:-0}"; fi; done'
    local INNER_REFRESH='BT=$(awk '\''/btime/ {print $2; exit}'\'' /proc/stat); UMS=$(awk '\''{printf "%d",$1*1000}'\'' /proc/uptime); TS=$(( BT*1000 + UMS )); PIDS=$(for p in /proc/[0-9]*; do bn=${p##*/}; f="/proc/$bn/comm"; [ -r "$f" ] || continue; c=$(cat "$f"); echo "$c" | grep -Eiq "$PROC_MATCH" || continue; echo -n "$bn "; done); for pid in $PIDS; do if [ -r /proc/$pid/stat ]; then LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; RSS=$( (grep -m1 "Rss:" /proc/$pid/smaps_rollup 2>/dev/null | tr -s " " | cut -d" " -f2) || true ); if [ -z "$RSS" ]; then PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null) && RSS=$(( ${PAGES:-0} * 4 )); fi; printf "{\"ts_ms\":%s,\"pid\":%s,\"rss_kb\":%s,\"utime\":%s,\"stime\":%s}\n" "$TS" "$pid" "${RSS:-0}" "${UT:-0}" "${ST:-0}"; fi; done'
    ...
    local H_STATIC='BT=$(awk '\''/btime/ {print $2; exit}'\'' /proc/stat); UMS=$(awk '\''{printf "%d",$1*1000}'\'' /proc/uptime); TS=$(( BT*1000 + UMS )); for pid in '$PIDS_HOST'; do if [ -r /proc/$pid/stat ]; then LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; RSS=$( (grep -m1 "Rss:" /proc/$pid/smaps_rollup 2>/dev/null | tr -s " " | cut -d" " -f2) || true ); if [ -z "$RSS" ]; then PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null) && RSS=$(( ${PAGES:-0} * 4 )); fi; printf "{\"ts_ms\":%s,\"pid\":%s,\"rss_kb\":%s,\"utime\":%s,\"stime\":%s}\n" "$TS" "$pid" "${RSS:-0}" "${UT:-0}" "${ST:-0}"; fi; done'
    local H_REFRESH='BT=$(awk '\''/btime/ {print $2; exit}'\'' /proc/stat); UMS=$(awk '\''{printf "%d",$1*1000}'\'' /proc/uptime); TS=$(( BT*1000 + UMS )); PIDS=$(for p in /proc/[0-9]*; do bn=${p##*/}; f="/proc/$bn/comm"; [ -r "$f" ] || continue; c=$(cat "$f"); echo "$c" | grep -Eiq "$PROC_MATCH" || continue; echo -n "$bn "; done); for pid in $PIDS; do if [ -r /proc/$pid/stat ]; then LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; RSS=$( (grep -m1 "Rss:" /proc/$pid/smaps_rollup 2>/dev/null | tr -s " " | cut -d" " -f2) || true ); if [ -z "$RSS" ]; then PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null) && RSS=$(( ${PAGES:-0} * 4 )); fi; printf "{\"ts_ms\":%s,\"pid\":%s,\"rss_kb\":%s,\"utime\":%s,\"stime\":%s}\n" "$TS" "$pid" "${RSS:-0}" "${UT:-0}" "${ST:-0}"; fi; done'
    if [ "${PROC_REFRESH}" = "1" ]; then
