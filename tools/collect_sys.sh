...
stop_collectors() {
  ...
  # 3) Optional global fallback: kill any stray samplers regardless of RUN_ID when STOP_ALL=1
  if [ "${STOP_ALL:-0}" = "1" ] || [ "${CLEAN_ALL:-0}" = "1" ]; then
    pkill -f 'mpstat 1$' 2>/dev/null || true
    pkill -f 'ifstat -i .* -t 1' 2>/dev/null || true
    pkill -f 'vmstat -Sm -t 1' 2>/dev/null || true
  fi
...
start_procmon() {
  ...
  if [ -n "$PID_DIR" ] && [ -d "$PID_DIR" ]; then
    ...
    nohup bash -c 'while :; do TS=$(date +%s%3N); for f in "'$PID_DIR'"/*; do [ -e "$f" ] || break; bn=${f##*/}; pid=$bn; if [ -r "/proc/$pid/comm" ]; then c=$(cat "/proc/$pid/comm" 2>/dev/null); echo "$c" | grep -Eiq "'$PROC_MATCH'" || { rm -f "$f" 2>/dev/null || true; continue; }; if [ -r "/proc/$pid/stat" ]; then LINE=$(cat "/proc/$pid/stat" 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; PAGES=$(cut -d" " -f2 "/proc/$pid/statm" 2>/dev/null); [ -n "$PAGES" ] || PAGES=0; RSS=$(( PAGES * 4 )); echo "{\"ts_ms\":$TS,\"pid\":$pid,\"rss_kb\":$RSS,\"utime\":$UT,\"stime\":$ST}"; fi; fi; done >> "'$OUT'" 2>>"'$LOG_DIR'/procmon.err"; sleep "'$INTERVAL_S'"; done' >/dev/null 2>&1 &
    echo $! > "$LOG_DIR/procmon.pid"
    echo "proc sampler started (mode=host, whitelist=$PID_DIR, interval=${INTERVAL_S}s) → $OUT"
    return
  fi
...
PIDS=""; for p in /proc/[0-9]*; do bn=${p##*/}; f="/proc/$bn/comm"; [ -r "$f" ] || continue; c=$(cat "$f"); echo "$c" | grep -Eiq "$PROC_MATCH" || continue; PIDS="$PIDS $bn"; done; echo ${PIDS# }' 2>/dev/null || true)
    [ -z "$PIDS" ] && PIDS="1"
    local INNER_STATIC='TS=$(date +%s%3N); for pid in '$PIDS'; do [ -r /proc/$pid/stat ] || continue; LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null); [ -n "$PAGES" ] || PAGES=0; RSS=$(( PAGES * 4 )); echo "{\"ts_ms\":$TS,\"pid\":$pid,\"rss_kb\":$RSS,\"utime\":$UT,\"stime\":$ST}"; done'
    local INNER_REFRESH='TS=$(date +%s%3N); PIDS=$(for p in /proc/[0-9]*; do bn=${p##*/}; f="/proc/$bn/comm"; [ -r "$f" ] || continue; c=$(cat "$f"); echo "$c" | grep -Eiq "$PROC_MATCH" || continue; echo -n "$bn "; done); for pid in $PIDS; do [ -r /proc/$pid/stat ] || continue; LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null); [ -n "$PAGES" ] || PAGES=0; RSS=$(( PAGES * 4 )); echo "{\"ts_ms\":$TS,\"pid\":$pid,\"rss_kb\":$RSS,\"utime\":$UT,\"stime\":$ST}"; done'
    ...
    local H_STATIC='TS=$(date +%s%3N); for pid in '$PIDS_HOST'; do [ -r /proc/$pid/stat ] || continue; LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null); [ -n "$PAGES" ] || PAGES=0; RSS=$(( PAGES * 4 )); echo "{\"ts_ms\":$TS,\"pid\":$pid,\"rss_kb\":$RSS,\"utime\":$UT,\"stime\":$ST}"; done'
