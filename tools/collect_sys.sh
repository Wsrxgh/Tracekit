...
PIDS=""; for p in /proc/[0-9]*; do bn=${p##*/}; f="/proc/$bn/comm"; [ -r "$f" ] || continue; c=$(cat "$f"); echo "$c" | grep -Eiq "$PROC_MATCH" || continue; PIDS="$PIDS $bn"; done; echo ${PIDS# }' 2>/dev/null || true)
    [ -z "$PIDS" ] && PIDS="1"
    local INNER_STATIC='TS=$(date +%s%3N); for pid in '$PIDS'; do [ -r /proc/$pid/stat ] || continue; LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null); [ -n "$PAGES" ] || PAGES=0; RSS=$(( PAGES * 4 )); TH=0; if [ "${PROC_THREADS:-0}" = "1" ]; then TH=$(grep -m1 "^Threads:" /proc/$pid/status 2>/dev/null | cut -f2); TH=${TH:-0}; fi; echo "{\"ts_ms\":$TS,\"pid\":$pid,\"rss_kb\":$RSS,\"utime\":$UT,\"stime\":$ST,\"threads\":$TH}"; done'
    ...
    local H_STATIC='TS=$(date +%s%3N); for pid in '$PIDS_HOST'; do [ -r /proc/$pid/stat ] || continue; LINE=$(cat /proc/$pid/stat 2>/dev/null) || continue; REST=${LINE#*) }; set -- $REST; UT=${12}; ST=${13}; PAGES=$(cut -d" " -f2 /proc/$pid/statm 2>/dev/null); [ -n "$PAGES" ] || PAGES=0; RSS=$(( PAGES * 4 )); TH=0; if [ "${PROC_THREADS:-0}" = "1" ]; then TH=$(grep -m1 "^Threads:" /proc/$pid/status 2>/dev/null | cut -f2); TH=${TH:-0}; fi; echo "{\"ts_ms\":$TS,\"pid\":$pid,\"rss_kb\":$RSS,\"utime\":$UT,\"stime\":$ST,\"threads\":$TH}"; done'
